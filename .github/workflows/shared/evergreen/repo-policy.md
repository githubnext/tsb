...
- Draft PR policy: **mark draft PRs ready for review** once all configured merge
  gates pass, using the `mark-pull-request-as-ready-for-review` safe output.
  Never mark a draft ready while any gate is failing, pending, or missing.
...
- Missing check policy: CI runs on `push` to `autoloop/**` and `pull_request` to
  ...
  CI; prefer `GH_AW_CI_TRIGGER_TOKEN` for CI-triggering pushes.
...
- Action token: `GH_AW_CI_TRIGGER_TOKEN` (already configured as a repo secret).
  This PAT-style token is used for CI-triggering pushes because default-token
  pushes may not trigger required CI. gh-aw wires it automatically into the
  safe-output push handler; the deterministic trigger falls back to
...
- Last 50 closed PR process scan: PRs merged via GitHub (auto-merge enabled, no
  ...
  CI-trigger token (`GH_AW_CI_TRIGGER_TOKEN` exists). Several prior Evergreen
  install/remove PRs (#357, #358, #366, #375) exist; this install supersedes
  them.
- Uncertainties: none blocking. All required secrets (`GH_AW_CI_TRIGGER_TOKEN`,
  `COPILOT_GITHUB_TOKEN`) and all `evergreen*` labels already exist.
