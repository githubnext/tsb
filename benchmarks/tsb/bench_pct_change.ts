...
for (let i = 0; i < 3; i++) s.pctChange();
const N = 200;
const t0 = performance.now();
for (let i = 0; i < N; i++) s.pctChange();
const elapsed = performance.now() - t0;
console.log(JSON.stringify({ function: "pct_change", mean_ms: elapsed / N, iterations: N, total_ms: elapsed }));
