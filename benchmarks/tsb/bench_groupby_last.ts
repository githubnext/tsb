...
const WARMUP = 3;
const ITERS = 20;
for (let i = 0; i < WARMUP; i++) gbObj.last();
const t0 = performance.now();
for (let i = 0; i < ITERS; i++) gbObj.last();
const total = performance.now() - t0;
console.log(JSON.stringify({ function: "groupby_last", mean_ms: total / ITERS, iterations: ITERS, total_ms: total }));
