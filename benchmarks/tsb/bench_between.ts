...
for (let i = 0; i < 3; i++) s.between(-1.0, 1.0);
const N = 200;
const t0 = performance.now();
for (let i = 0; i < N; i++) s.between(-1.0, 1.0);
const elapsed = performance.now() - t0;
console.log(JSON.stringify({ function: "between", mean_ms: elapsed / N, iterations: N, total_ms: elapsed }));
