...
for (let i = 0; i < 3; i++) s.rolling(20).std();
const N = 50;
const t0 = performance.now();
for (let i = 0; i < N; i++) s.rolling(20).std();
const elapsed = performance.now() - t0;
console.log(JSON.stringify({ function: "rolling_std", mean_ms: elapsed / N, iterations: N, total_ms: elapsed }));
