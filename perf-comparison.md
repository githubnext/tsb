...
Next functions to benchmark (iter 42+):
1. More `describe` variants (percentiles, include='all')
2. `DataFrame.melt()` with custom var_name/value_name
3. Additional `rolling` window functions (ewm on DataFrame)
4. `Series.str` accessor - more text processing variants
5. Index set operations (union/intersection/difference)
...
- **Status**: ✅ Accepted
- **Change**: Added 206 new benchmark pairs from main (22-pair base) to reach 228 total. Series stats (std/var/median/quantile/corr/nunique/isin/eq/gt/lt/between/idxmax/idxmin/skew/kurt/sem/count/sum/mean/min/max/isna/notna/dropna/unique/loc/iloc), Series standalone (cumprod/cummax/cummin/abs/round/clip/rank/nlargest/nsmallest/where/mask/apply/transform/diff/pct_change/map/sample/autocorr/loc_range/iloc_range), DataFrame ops (abs/round/clip/cumsum/cummax/cummin/cumprod/where/transform/rank/apply_map/diff/pct_change/replace/sample/nlargest/nsmallest/value_counts/corr/cov/apply_col/transform_rows), Rolling (sum/min/max/count/std/var/median/sem/skew/kurt/quantile/apply), Expanding (sum/mean/std/var/max/min/count/median), EWM (std/var/corr/cov), DataFrame (rolling/expanding/ewm mean/sum/std), GroupBy (sum/count/std/min/max/size/first/last/transform/agg/nunique/var/median/apply), Stats (zscore/normalize/cv/percentile_of_score/digitize/histogram/linspace/arange/pearson_corr/cut/qcut/series_digitize), Categorical (from_codes/sort_by_freq/freq_table/recode/to_ordinal/union/intersect/diff/cross_tab), Format (float/percent/scientific/thousands/currency/compact/series_to_string/df_to_string), Strings (normalize/get_dummies/remove_prefix/remove_suffix/split_expand/partition/rpartition/multi_replace/indent/dedent/extract_groups/translate/char_width/byte_length/extract_all), Reshape (melt/stack/unstack/pivot/wide_to_long), IO (to_csv/to_json/read_json), DataFrame struct (insert_column/pop_column/reorder_columns/move_column/df_from_pairs), Dict (to_dict/from_dict), Datetime (dt_year/month/day/hour/dayofweek), Type checks (is_scalar/number/float/integer/string/missing/list_like), Attrs (get_set/update/copy/merge), Pipe (pipe/df_apply_col/df_transform_rows), isna/notna standalone, Merge (left/outer), Concat (axis1), MultiIndex (create/from_tuples), RangeIndex (create/slice), describe (series/df), quantile_multi, value_counts_normalize.
- **Metric**: 228 (previous best: 207, delta: +21) | **Commit**: 209f3ef
- **Notes**: Branch created fresh from main (22-pair base). Comprehensive generator added all Future Directions from iter 40: sample, autocorr, ewm corr/cov, describe, quantile_multi, value_counts_normalize, DataFrame rolling/expanding/ewm variants.
