...
export class DataFrame {
  ...
  *iterrows(): IterableIterator<[Label, Series<Scalar>]> {
   ...
   *
   * Aggregations are applied independently to each column, accumulating all
   * observations from the start up to and including the current row.
   *
...
export class DataFrameRolling {
  ...
  private _applyColAgg(
    ...
    for (const colName of this._df.columns.values) {
      ...
      colMap.set(colName, new Series<Scalar>({ data: result.values, index: col.index, name: result.name }));
    }
 ...
 *
 * Aggregations are applied independently to each numeric column, accumulating
 * all observations from the first row up to the current row.
 *
